        ...
        if (Test-Path $r) {
            foreach ($k in (Get-ChildItem $r -ErrorAction SilentlyContinue)) {
                ...
                if ($bin -and (Test-Path $bin)) {
                    $exe = Join-Path $bin '1cestart.exe'
...
function Invoke-Pipeline {
    ...
    $ts  = Get-Date -Format 'yyyy_MM_dd_HH_mm'
    ...
    try {
        if ($type -eq 'DT') {
            ...
            if (-not $exe) { throw "Не найден исполняемый файл 1С (ни ExePath из конфига, ни 1cestart.exe из реестра)" }
            ...
            $tempLogFile = Join-Path $env:TEMP ("1c_designer_{0}.log" -f [System.Guid]::NewGuid().ToString())

            $srcF83 = Get-ShortPath $srcFolder
            $argsF  = BuildArgsLine-F $srcF83 $artifact $tempLogFile $login $password
            $procF  = Start-Process -FilePath $exe -ArgumentList $argsF -WindowStyle Hidden -PassThru
            
            # Ждём с таймаутом 60 минут
            $ok = Wait-FileStable -Path $artifact -Process $procF -TimeoutSec 3600 -PostExitWaitSec 300
            [void]$procF.WaitForExit()
            
            if ($procF.ExitCode -ne 0) { 
                & $log ("[WARN] 1cv8 завершился с кодом {0}" -f $procF.ExitCode) 
            }

            # Читаем лог конфигуратора только при ошибке
            $designerError = $null
            if (-not $ok -and (Test-Path $tempLogFile)) {
                try {
                    $designerLog = Get-Content $tempLogFile -Encoding UTF8 -ErrorAction SilentlyContinue | Select-Object -Last 10
                    if ($designerLog) {
                        $designerError = ($designerLog -join "; ").Substring(0, [Math]::Min(500, ($designerLog -join "; ").Length))
                    }
                } catch {}
            }
            
            # Всегда удаляем временный лог
            ...
            if (-not $ok) {
                if ($designerError) { 
                    throw "Не удалось получить стабильный файл .dt за 60 минут. Ошибка: $designerError" 
                }
                else { 
                    throw "Не удалось получить стабильный файл .dt за 60 минут" 
                }
            }
        }
        elseif ($type -eq '1CD') {
